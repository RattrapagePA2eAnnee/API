...
function getReservations(?array $columns = null): array
{
    if (!is_array($columns)) {
        $columns = [];
    }
    ...
    $authorizedColumns = ["id", "reservation_date", "total_price", "user_id", "status"];
    ...
    $getUserQuery = $databaseConnection->prepare("SELECT * FROM RESERVATIONS WHERE $whereClause;");
