...
function getLessons(?array $columns = null): array
{
    if (!is_array($columns)) {
        $columns = [];
    }
    ...
    $authorizedColumns = ["id", "description", "content"];
    ...
    $getUserQuery = $databaseConnection->prepare("SELECT * FROM LECON WHERE $whereClause;");
